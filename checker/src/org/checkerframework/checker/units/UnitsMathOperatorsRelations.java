...
/**
 * This class encodes the units checker's type rules for all math arithmetic operators, compound
 * assignment operators, and comparison operators.
 *
 * <p>There are 2 main methods in this class: {@link #processMathOperation(ExpressionTree,
 * Tree.Kind, AnnotatedTypeMirror, AnnotatedTypeMirror, AnnotatedTypeMirror)} and {@link
 * #processCompoundAssignmentOperation(CompoundAssignmentTree, Tree.Kind, AnnotatedTypeMirror,
 * AnnotatedTypeMirror, AnnotatedTypeMirror)}
 *
 * <p>Both methods are designed with {@link Tree.Kind} parameter to allow for flexibility in
 * applying the rules in methods that perform these calculations and comparisons, but whereas the
 * AST node's Kind is not necessarily the specific operators. See each method's javadoc for
 * examples.
 *
 * @author jeff luo
 */
public class UnitsMathOperatorsRelations {
    ...
    /**
     * This method will assign a result type for a given arithmetic or comparison operation based on
     * the given kind, the left hand type, and right hand type. Errors may also be issued on the
     * given AST node if there are any rule violations.
     *
     * <p>It is called at {@code UnitsPropagationTreeAnnotator.visitBinary()} for checking
     * operators.
     *
     * <p>It is also designed to support arithmetic or comparison behavior in various methods such
     * as {@code Integer.sum(int, int)}, {@code Math.multiplyExact(int, int)}, and {@code
    ...
    /**
     * This helper method encodes the rules for subtraction, which is also shared by addition.
     *
     * @param resultType the result unit
    ...
    private void processMinus(
        ...
        if (UnitsRelationsTools.hasSpecificUnit(lht, factory.TOP)
            ...
            // otherwise we return the LUB of the left and right
            // types
     ...
     *
     * <p>Note that multiplication rules encoded in a {@link UnitsRelations} subclass precedes these
     * rules.
     *
     * @param resultType the result unit
     ...
     *
     * <p>Note that division rules encoded in a {@link UnitsRelations} subclass precedes these
     * rules.
     *
     * @param resultType the result unit
     ...
     *
     * <p>Comparisons are allowed between a unit and scalar at any time, and between a unit and
     * UnitsBottom (type of null reference). Otherwise, comparisons must be between exactly the same
     * units.
     *
     * @param resultType the result unit, which is set to scalar if the comparison check passes
    ...
    // intended
    /**
     * This method encodes the rules for checking a given compound assignment operation based on the
     * given kind, the left hand variable type, and right hand expression type. Errors may also be
     * issued on the given AST node if there are any rule violations.
     *
     * <p>It is called at two locations for checking operators: {@code
     * UnitsPropagationTreeAnnotator.visitCompoundAssignment()} and {@link
     * UnitsVisitor#visitCompoundAssignment(CompoundAssignmentTree, Void)} . In the former, a
     * resultType is also passed in where the logic here updates the result type accordingly. In the
     * latter, resultType passed in is null.
     *
     * <p>It is also designed to support compound assignment behavior in boxed types (or more
     * generally any method) such as {@link AtomicLong#getAndAdd(long)}.
     *
    ...
    /**
     * Uses a units relations class to resolve and return an annotation mirror representing the type
     * of the result of a calculation, or null if this units relations class doesn't specify what
     * type to return
     *
     ...
     * @return an AnnotationMirror representing the type of the result of a calculation, or null if
     *     this units relations class doesn't specify what type to return
     */
    ...
    /**
     * This helper method encodes the rules for adding a time duration to a time instant. E.g. 5 am
     * + 5 hours = 10 am
     *
     * <p>If the time instant is based on the same unit as the duration, then it will return the
     * {@link TimeInstant} unit.
     *
     * @param resultType the result unit
    ...
    /**
     * Checks to see if the units annotation in the annotated type mirror is a subtype of {@link
     * TimeDuration}
     *
    ...
    /**
     * This helper method checks to see if the units annotation in the annotated type mirror is a
     * subtype of superType. In Units checker, each annotated type mirror can only have 1 units
     * annotation at any given time
     *
     * @param atm annotated type mirror with a units annotation
    ...
    /**
     * Returns the set of least upper bounds of the two annotated types through the type factory
     *
     * @param lht left type
     * @param rht right type
    ...
    /**
     * Returns the least upper bound of two annotation mirrors through the type factory
     *
