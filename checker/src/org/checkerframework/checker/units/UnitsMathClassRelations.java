...
public class UnitsMathClassRelations extends UnitsClassRelations {
    ...
    public void processMethodInvocation(
        ...
        switch (methodName) {
            case "addExact":
                processMathLibraryArithmeticOperation(
                        node, Tree.Kind.PLUS, resultType, methodArguments);
                break;
            case "subtractExact":
                processMathLibraryArithmeticOperation(
                        node, Tree.Kind.MINUS, resultType, methodArguments);
                break;
            case "multiplyExact":
                processMathLibraryArithmeticOperation(
                        node, Tree.Kind.MULTIPLY, resultType, methodArguments);
                break;
            case "floorDiv":
                processMathLibraryArithmeticOperation(
                        node, Tree.Kind.DIVIDE, resultType, methodArguments);
                break;
            case "IEEEremainder":
                processMathLibraryArithmeticOperation(
                        node, Tree.Kind.REMAINDER, resultType, methodArguments);
                break;
            case "atan2":
                ...
                if (!UnitsRelationsTools.areSameUnits(lht, rht)) {
                    checker.report(
                            Result.failure(
                                    "two.parameter.method.arguments.unit.mismatch",
                                    lht.toString(),
                                    rht.toString()),
                            node);
                }
    ...
    /**
     * If arg is one of the supported area units, then it will return the corresponding length unit
     *
     * @param arg a unit which may or may not be one of the supported area units
     * @param type the result unit, which will be replaced with the corresponding length unit if arg
    ...
    /**
     * If arg is one of the supported volume units, then it will return the corresponding length
     * unit
     *
     * @param arg a unit which may or may not be one of the supported volume units
     * @param type the result unit, which will be replaced with the corresponding length unit if arg
    ...
    /**
     * This method will first attempt to evaluate powExponent down to a double value, then depending
     * on the value of the exponent, it will produce a unit for the resultType depending on the unit
     * of powBase
     *
    ...
    private void processPower(
            AnnotatedTypeMirror powBase,
            ExpressionTree powExponent,
            AnnotatedTypeMirror resultType) {
        double exp;
    ...
    /**
     * Attempts to evaluate a mathematical expression (passed in as root) into a double value. The
     * method returns the double value and true as a pair if the evaluation was successful,
     * otherwise it returns 0.0 and false as a pair.
     *
     * <p>Expressions with any non number literals will not be successfully evaluated.
     *
     * @param root the root node in the expression tree of a mathematical expression
